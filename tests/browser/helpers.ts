 ...
 */
export async function waitForElement(page: any, selector: string, timeout = 5000): Promise<any> {
 ...
 */
export async function uploadFile(page: any, fileInputSelector: string, filePath: string): Promise<void> {
 ...
 */
export async function hasCorsError(page: any, consoleMessages: string[] = []): Promise<boolean> {
    ...
    const outputErrorPatterns = ["error:", "✗ error", "failed", "network"];
 ...
 */
export async function waitForWebSocketLogs(page: any, timeout = 30000): Promise<boolean> {
    const startTime = Date.now();
...
export async function findRunButton(page: any): Promise<string | null> {
    const buttonIds = ["runExample", "startButton", "listButton", "getButton", "transcribeButton"];
