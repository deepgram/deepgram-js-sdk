...
`reference.md` is Fern-generated from HTTP endpoint definitions and intentionally covers REST methods only. Do not edit or freeze it to add WebSocket documentation. The packaged `README.md` owns the streaming method index; update that index whenever `CustomClient.ts` gains a streaming resource or a generated socket method is added, removed, or renamed.
...
- `tests/unit/*` — hand-written unit tests (includes `compat-aliases.test.ts`, the regression test that proves the legacy `CreateKeyV1RequestOne` type still resolves to `CreateKeyV1Request`, and `regen-constraints.test.ts`, which exercises the runtime behavior of the compat shims — both must be listed explicitly in `.fernignore` or the generator deletes them). Also covers the coverage-hardening suites added alongside them: `core-branches.test.ts` (guards the `src/Client.ts` passthrough baseUrl patch by asserting the resolved host), `rest-clients.test.ts` (asserts `BadRequestError`/`DeepgramError` classes and query-param serialization across every generated REST method), `speak-v2-coverage.test.ts` (Speak V2 Flux TTS connect/socket/batch surface), and `websocket/ws.test.ts` (guards the `src/core/websocket/ws.ts` reconnect-policy patch).
...
- `package.json` — Fern's generator produces a minimal `devDependencies` set; we add the following:
  ...
  - `@vitest/coverage-v8` — required by `make test-coverage`, which is the CI test gate (`.github/workflows/ci.yml`). Without it that job fails at the `--coverage` flag. Its version must track `vitest` exactly (peer dependency, and `.npmrc` sets `strict-peer-dependencies=true`), so **both are exact-pinned to the same version and must be bumped together** — a caret on `vitest` would let it float to a new minor while coverage-v8 stayed fixed, which is a hard peer mismatch and a failed install. Note `vitest` itself is emitted by Fern with a caret range; re-pinning it to the exact version is part of this patch.
...
- `src/api/types/SpeakV2Speed.ts` — the 2026-09-07 regen corrected this type to `number`, but dropped the named string constants shipped in 5.10.0. Existing callers using `SpeakV2Speed.One` would fail to compile; retain the constants and a `string | number` type until the next major release. Regression coverage in `tests/unit/regen-constraints.test.ts`.
- `src/api/resources/agent/resources/v1/client/Socket.ts`, `src/api/resources/listen/resources/v1/client/Socket.ts`, `src/api/resources/listen/resources/v2/client/Socket.ts`, `src/api/resources/speak/resources/v1/client/Socket.ts`, and `src/api/resources/speak/resources/v2/client/Socket.ts` — Fern 3.94.0 changes `on()` from replacement to accumulation. Retain replacement semantics and isolate throwing user callbacks through v5, while keeping the generated `off()` API, then revisit at the next major. Listen V2 also retains the hand-written `sendForceEndTurn()` server-enablement caveat: deployments without the feature return `UNPARSABLE_CLIENT_MESSAGE` and close the socket.
- `src/core/websocket/ws.ts` — Fern 3.94.0 evaluates `shouldReconnect` while an explicit `reconnect()` performs its internal close. A callback returning false then vetoes the explicit reconnect; re-arm the reconnect flag after that close so caller-directed reconnects always connect. Regression coverage: `tests/unit/websocket/ws.test.ts`.
