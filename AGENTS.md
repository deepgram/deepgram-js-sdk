...
- `src/api/resources/listen/resources/v2/client/Socket.ts` — retain the hand-written `sendForceEndTurn()` server-enablement caveat: deployments without the feature return `UNPARSABLE_CLIENT_MESSAGE` and close the socket. The file is generated, so freeze it and reapply this documentation patch after each regen.
