...
- `src/version.ts`, `src/BaseClient.ts` — carry the SDK version string that release-please bumps on release (both are declared as generic `extra-files` in `.github/release-please-config.json`). release-please's generic updater needs an inline `// x-release-please-version` marker to know which line to bump, but Fern emits these files without a marker and re-stamps an arbitrary version each regen — so release-please couldn't track them and the version drifted (e.g. `version.ts` sat at 5.2.1 while the release was 5.4.0). We add the `// x-release-please-version` markers (pinned to the released version) and freeze the files so the markers survive regen. Re-apply the markers if a regen ever overwrites these.
