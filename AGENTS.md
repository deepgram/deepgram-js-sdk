...
- `src/core/websocket/ws.ts` — the reconnect loop sets `_connectLock = true` before checking whether `maxRetries` has been exhausted, then returns without clearing it. Consumers waiting on an async iterator consequently never receive terminal completion after a recoverable close when no retries remain. Clear the lock before that return so wrapper-level async iteration can distinguish a pending reconnect from exhaustion. **Report upstream to Fern** — this is a generator bug. Regression coverage in `tests/wire/websocket/async-iteration.test.ts`.
