...
- `src/api/resources/agent/resources/v1/types/AgentV1UpdateListen.ts` — backward-compat shim for the `Listen.provider` field. The 2026-07-31 regen repointed it from `DeepgramListenProviderV2` to the new `AgentV1UpdateListenListenProvider` union, whose `V1`/`V2` variants both redeclare the `version` discriminant as **required**. Because `DeepgramListenProviderV2.version` is optional, existing callers that omitted it (`{ type: "deepgram", model: "flux-general-en" }`) stopped compiling with TS2322 — a breaking change on a published send path (`agent.v1` → `sendUpdateListen`). We union the bare `DeepgramListenProviderV2` back in, so old call sites keep working while the generated union still offers the explicit v1/v2 forms. Compile-compat only — the serialized payload is unchanged. The upstream spec actually *relaxed* this field (its docs dropped "the provider identity (type, version, model) is required and must match the current session"), so the required discriminant looks like a Fern modeling artifact; drop this entry when the generator stops requiring it. Regression coverage in `tests/unit/regen-constraints.test.ts`.
