...
- `src/api/resources/agent/resources/v1/types/AgentV1UpdateListen.ts` — backward-compat shim for the `Listen.provider` field. The 2026-07-31 regen repointed it from `DeepgramListenProviderV2` to the new `AgentV1UpdateListenListenProvider` union, whose `V1`/`V2` variants both redeclare the `version` discriminant as **required**. Because `DeepgramListenProviderV2.version` is optional, existing callers that omitted it (`{ type: "deepgram", model: "flux-general-en" }`) stopped compiling with TS2322 — a breaking change on a published send path (`agent.v1` → `sendUpdateListen`). We replace the field with a single merged shape that never references the generated union — `Omit<DeepgramListenProviderV2, "version"> & { version?: string; language?: string; smart_format?: boolean }` — so old call sites keep working. A union can't fix this: property access on a union requires the field on *every* arm, so reads of the V2-only fields (`eot_threshold`, `eager_eot_threshold`, `eot_timeout_ms`, `language_hints`) would break with TS2339 on the `V1` arm. The merged shape derives from `DeepgramListenProviderV2` (keeping every previously-readable field readable and `model` required), widens `version` to open `string` for the v1/v2 selection, and hand-adds the two V1-only fields (`language`, `smart_format`) as optional. Compile-compat only — the serialized payload is unchanged. The upstream spec actually *relaxed* this field (its docs dropped "the provider identity (type, version, model) is required and must match the current session"), so the required discriminant looks like a Fern modeling artifact; drop this entry when the generator stops requiring it. Regression coverage in `tests/unit/regen-constraints.test.ts`.
