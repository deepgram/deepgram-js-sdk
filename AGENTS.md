...
- `src/BaseClient.ts` — carries two patches: derive the SDK version from `src/version.ts` (`import { SDK_VERSION }` + `` `@deepgram/sdk/${SDK_VERSION}` ``) instead of Fern's hardcoded version string, and wrap the generated API-key provider with a process-safe supplier so browser/edge runtimes without Node's `process` global raise the normal `DeepgramError`. After regen, re-apply both patches and take all of Fern's other BaseClient changes.
