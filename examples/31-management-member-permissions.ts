...
async function listProjectMembers() {
    ...
    try {
        const data = await deepgramClient.manage.v1.projects.members.list(projectId);
...
async function getMemberScopes(memberId) {
    ...
    try {
        const data = await deepgramClient.manage.v1.projects.members.scopes.list(projectId, memberId);
...
async function promoteToAdmin(memberId) {
    ...
    try {
        const data = await deepgramClient.manage.v1.projects.members.scopes.update(projectId, memberId, {
            scope: "admin",
        });
...
async function listPendingInvites() {
    ...
    try {
        const data = await deepgramClient.manage.v1.projects.members.invites.list(projectId);
...
async function inviteNewMember() {
    ...
    try {
        const data = await deepgramClient.manage.v1.projects.members.invites.create(projectId, {
...
async function revokeInvitation() {
    ...
    try {
        await deepgramClient.manage.v1.projects.members.invites.delete(projectId, inviteEmail);
...
async function removeMemberFromProject(memberId) {
    ...
    try {
        await deepgramClient.manage.v1.projects.members.delete(projectId, memberId);
...
async function auditMemberPermissions() {
    ...
    try {
        const members = await deepgramClient.manage.v1.projects.members.list(projectId);

        console.log("=== Permission Audit Report ===\n");

        for (const member of members.members || []) {
            try {
                const scopes = await deepgramClient.manage.v1.projects.members.scopes.list(projectId, member.member_id);
...
async function manageTeamWorkflow() {
    ...
    try {
        ...
        const invites = await deepgramClient.manage.v1.projects.members.invites.list(projectId);
...
async function checkMemberPermission(memberId, requiredScope) {
    ...
    try {
        const scopes = await deepgramClient.manage.v1.projects.members.scopes.list(projectId, memberId);
