...
async function listKeys() {
    try {
        const data = await deepgramClient.manage.v1.projects.keys.list(projectId);
...
async function getKey(keyId) {
    try {
        const data = await deepgramClient.manage.v1.projects.keys.get(projectId, keyId);
...
async function createKey() {
    try {
        const data = await deepgramClient.manage.v1.projects.keys.create(projectId, {
            comment: "My API key",
